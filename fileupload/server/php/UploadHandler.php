...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		if(isset($_SESSION['yearclicked']) && isset($_SESSION['monthclicked']) && isset($_SESSION['dayclicked'])){
				...
				//unset($_SESSION['yearclicked']);
				//unset($_SESSION['monthclicked']);
				//unset($_SESSION['dayclicked']);
		}
		
		//$source = 'files/.htaccess';
		//$destination_album = dirname($this->get_server_var('SCRIPT_FILENAME')).'../../../../accounts/'.$account.'/'.$currentyear.'/'.$currentmonth.'/'.$currentday.'/album/.htaccess';
		//$destination_thumbs = dirname($this->get_server_var('SCRIPT_FILENAME')).'../../../../accounts/'.$account.'/'.$currentyear.'/'.$currentmonth.'/'.$currentday.'/thumbs/.htaccess';
		//if(!is_file($destination_album) ){
			//copy($source,$destination_album);			
		//}
		//if(!is_file($destination_thumbs)){
			//copy($source,$destination_thumbs);		
		//}
	        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'../../../../accounts/'.$account.'/'.$currentyear.'/'.$currentmonth.'/'.$currentday.'/album/',
			'upload_url' => $this->get_full_url().'../../../../accounts/'.$account.'/'.$currentyear.'/'.$currentmonth.'/'.$currentday.'/album/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'image_versions' => array(
                ...
                */
                ...
                'thumbnail' => array(
                    ...
                    'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'../../../../accounts/'.$account.'/'.$currentyear.'/'.$currentmonth.'/'.$currentday.'/thumbs/',
                    'upload_url' => $this->get_full_url().'../../../../accounts/'.$account.'/'.$currentyear.'/'.$currentmonth.'/'.$currentday.'/thumbs/',
